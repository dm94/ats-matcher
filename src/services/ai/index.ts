...
import { ToolChoice, generateObject, generateText } from 'ai';

export const createAiInstance = (config: AIConfig): OpenAIProvider => createOpenAI(config);

export const generateTextResponse = async (config: AIConfig, prompt: string, tools?: any, toolChoice: ToolChoice<any> = "auto"): Promise<string> => {
